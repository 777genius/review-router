...
export class ProviderRegistry {
  ...
  async createProviders(config: ReviewConfig): Promise<Provider[]> {
    ...
    if (providers.length === 0 && usingDefaults && !userProvidedList) {
      logger.info(
        '🔍 No providers specified, starting dynamic model discovery...'
      );
      ...
      if (discoveredModels.length > 0) {
        logger.info(
          `🎯 Total discovered: ${discoveredModels.length} free models`
        );
    ...
    let discoveryLimit =
      (config.providerDiscoveryLimit ?? 0) > 0
    ...
    const otherProviders = providers.filter(
      (p) =>
        !p.name.startsWith('openrouter/') && !p.name.startsWith('opencode/')
    );
  ...
  private instantiate(
    ...
    for (const name of names) {
      ...
      if (this.pluginLoader?.hasProvider(name)) {
        ...
        const apiKey =
          process.env[pluginEnvVar] || process.env.PLUGIN_API_KEY || '';
      ...
      if (name.startsWith('openrouter/')) {
        ...
        // Strip instance suffix (e.g., "free#1" -> "free") before passing the
        // model id to Codex/OpenRouter. The suffix remains in provider.name so
        // quorum identity and logs can distinguish repeated model instances.
    ...
    if (config.providerAllowlist.length > 0) {
      filtered = filtered.filter((provider) =>
        config.providerAllowlist.some((pattern) =>
          provider.name.includes(pattern)
        )
      );
    }
  ...
  private selectWithDiversity(
    ...
    const openrouterCount = selected.filter((p) =>
      p.name.startsWith('openrouter/')
    ).length;
    const opencodeCount = selected.filter((p) =>
      p.name.startsWith('opencode/')
    ).length;
  ...
  private adjustForDiversity(
    ...
    const openrouter = providers.filter((p) =>
      p.name.startsWith('openrouter/')
    );
