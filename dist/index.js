...
${generatedBlock}` : generatedBlock;
  ...
  summarizeSourceFile(file, patch) {
    ...
    const target = symbols.length > 0 ? symbols.slice(0, 2).join(", ") : this.basenameWithoutExtension(file.filename);
    if (topics.length > 0) {
...
${deleted}`;
    ...
    if (deleted.includes("?.") || deleted.includes("??") || /\bnull\b/.test(combined) || /\bfallback\b/.test(combined)) {
      topics.push("fallback/null handling");
    }
    if (patch.additions.some((line) => line.startsWith("return ")) || patch.deletions.some((line) => line.startsWith("return "))) {
